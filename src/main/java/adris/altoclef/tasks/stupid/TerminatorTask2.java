 ...
 */
public class TerminatorTask2 extends Task {
    ...
    private static Item[] GEAR_TO_COLLECT = new Item[]{
            Items.NETHERITE_PICKAXE, Items.NETHERITE_SHOVEL, Items.NETHERITE_SWORD
    };
    ...
    public TerminatorTask2(BlockPos center, double scanRadius, Predicate<PlayerEntity> canTerminate) {
        _canTerminate = canTerminate;
        _scanTask = new ScanChunksInRadius(center, scanRadius);
    }

    public TerminatorTask2(BlockPos center, double scanRadius) {
...
/*            else if (_closestPlayerLastPos.distanceTo(mod.getPlayer().getPos()) > 25 && AwarenessSystem.getThreatLevel() < 50) {
        ...
        if (_armorTask != null && _armorTask.isActive() && !_armorTask.isFinished(mod)) {
            setDebugState("Collecting Netherite Armor");
            return _armorTask;
        }
        ...
        if (!StorageHelper.isArmorEquippedAll(mod, ItemHelper.NETHERITE_ARMORS)) {
            _armorTask = new EquipArmorTask(ItemHelper.NETHERITE_ARMORS);
            return _armorTask;
        }
    ...
    protected boolean isEqual(Task other) {
        return other instanceof TerminatorTask2;
    }
    ...
    private boolean isReadyToPunk(AltoClef mod) {
        return StorageHelper.isArmorEquippedAll(mod, ItemHelper.NETHERITE_ARMORS) && mod.getItemStorage().hasItem(Items.NETHERITE_SWORD);
    }
